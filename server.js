...
app.use(cors({ origin: "http://localhost:3000" }));  // Change if your frontend URL is different
...
<div style="max-width: 600px; margin: auto; background: #ffffff; border-radius: 8px; box-shadow: 0px 4px 10px rgba(0, 0, 0, 0.1); font-family: Arial, sans-serif;">
  ...
  <div style="background: #f4f4f4; padding: 15px; text-align: center; font-size: 14px; color: #555; border-radius: 0 0 8px 8px;">
    Best regards,<br><strong>Manoj Kumar</strong><br><a href="mailto:${process.env.EMAIL_USER}" style="color: #007bff;">${process.env.EMAIL_USER}</a>
  </div>
</div>
`;
  ...
  try {
    await contactEmail.sendMail({
      ...
      to: "manojkumar65927@gmail.com",  // Your own email address
      replyTo: email,
      subject: "Contact Form Submission - Techfolio",
